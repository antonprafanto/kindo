...
class DeployController extends Controller
{
    ...
    public function seedArticle108Draft(): Response
    {
        $this->authorizeDeployHook();
        ...
        try {
            Artisan::call('db:seed', [
                '--class' => 'Database\\Seeders\\Article108Seeder',
                '--force' => true,
            ]);
        } catch (\Throwable $e) {
            report($e);

            return response('Article 108 draft seed failed: '.$e->getMessage(), 500);
        }

        $slug = 'fullstack-iot-pc-rules-nodered-mqtt';
        $article = Article::where('slug', $slug)->first();
        if (! $article || $article->status !== 'draft' || $article->published_at !== null || Article::published()->where('slug', $slug)->exists()) {
            return response('Article 108 must remain draft (pre-launch B)', 500);
        }

        $requiredId = ['#108 (ini)', 'FS-38', 'Node-RED', 'Ambang ada di PC, bukan di sketch.', 'FS38_device_only', 'Deploy', 'FS-39'];
        $missingId = array_values(array_filter($requiredId, fn (string $needle): bool => ! str_contains((string) $article->body, $needle)));
        if ($missingId !== []) {
            return response('Article 108 ID content checks failed: '.implode(', ', $missingId), 500);
        }

        $requiredEn = ['#108 (this article)', 'FS-38', 'Node-RED', 'Ambang ada di PC, bukan di sketch.', 'FS38_device_only', 'Deploy', 'FS-39'];
        $missingEn = array_values(array_filter($requiredEn, fn (string $needle): bool => ! str_contains((string) $article->body_en, $needle)));
        if (! filled($article->title_en) || ! filled($article->body_en) || ! filled($article->seo_title_en) || ! filled($article->seo_description_en) || $missingEn !== []) {
            return response('Article 108 EN content checks failed: '.implode(', ', $missingEn), 500);
        }

        if (! str_contains((string) $article->cover_image, 'fs38-cover-rules')) {
            return response('Article 108 cover check failed', 500);
        }
    ...
    public function seedArticle115Draft(): Response
    {
        ...
        try {
            Artisan::call('db:seed', [
                '--class' => 'Database\\Seeders\\Article115Seeder',
                '--force' => true,
            ]);
        } catch (\Throwable $e) {
            report($e);

            return response('Article 115 draft seed failed: '.$e->getMessage(), 500);
        }

        $slug = 'fullstack-iot-chartjs-histori-suhu-flask';
        $article = Article::where('slug', $slug)->first();
        if (! $article || $article->status !== 'draft' || $article->published_at !== null || Article::published()->where('slug', $slug)->exists()) {
            return response('Article 115 must remain draft (pre-launch B)', 500);
        }

        $requiredId = ['#115 (ini)', 'Grafik tampil.', '/history?hours=1', '12 titik satu jam siap.', 'FS-46'];
        $missingId = array_values(array_filter($requiredId, fn (string $needle): bool => ! str_contains((string) $article->body, $needle)));
        if ($missingId !== []) {
            return response('Article 115 ID content checks failed: '.implode(', ', $missingId), 500);
        }

        $requiredEn = ['#115 (this article)', 'Grafik tampil.', '/history?hours=1', '12 titik satu jam siap.', 'FS-46'];
        $missingEn = array_values(array_filter($requiredEn, fn (string $needle): bool => ! str_contains((string) $article->body_en, $needle)));
        if (! filled($article->title_en) || ! filled($article->body_en) || ! filled($article->seo_title_en) || ! filled($article->seo_description_en) || $missingEn !== []) {
            return response('Article 115 EN content checks failed: '.implode(', ', $missingEn), 500);
        }

        if (! str_contains((string) $article->cover_image, 'fs45-cover-chart')) {
            return response('Article 115 cover check failed', 500);
        }

        Artisan::call('view:clear');

        return response('Article 115 seeded as draft (pre-launch B)', 200);
    }

    public function seedArticle114Draft(): Response
    {
        $this->authorizeDeployHook();
        ...
        try {
            Artisan::call('db:seed', [
                '--class' => 'Database\\Seeders\\Article114Seeder',
                '--force' => true,
            ]);
        } catch (\Throwable $e) {
            report($e);

            return response('Article 114 draft seed failed: '.$e->getMessage(), 500);
        }

        $slug = 'fullstack-iot-html-dashboard-suhu-flask';
        $article = Article::where('slug', $slug)->first();
        if (! $article || $article->status !== 'draft' || $article->published_at !== null || Article::published()->where('slug', $slug)->exists()) {
            return response('Article 114 must remain draft (pre-launch B)', 500);
        }

        $requiredId = ['#114 (ini)', 'file://', 'Suhu tampil.', 'stasiun.db', 'FS-45'];
        $missingId = array_values(array_filter($requiredId, fn (string $needle): bool => ! str_contains((string) $article->body, $needle)));
        if ($missingId !== []) {
            return response('Article 114 ID content checks failed: '.implode(', ', $missingId), 500);
        }

        $requiredEn = ['#114 (this article)', 'file://', 'Suhu tampil.', 'stasiun.db', 'FS-45'];
        $missingEn = array_values(array_filter($requiredEn, fn (string $needle): bool => ! str_contains((string) $article->body_en, $needle)));
        if (! filled($article->title_en) || ! filled($article->body_en) || ! filled($article->seo_title_en) || ! filled($article->seo_description_en) || $missingEn !== []) {
            return response('Article 114 EN content checks failed: '.implode(', ', $missingEn), 500);
        }

        if (! str_contains((string) $article->cover_image, 'fs44-cover-dashboard')) {
            return response('Article 114 cover check failed', 500);
        }

        Artisan::call('view:clear');

        return response('Article 114 seeded as draft (pre-launch B)', 200);
    }

    public function seedArticle113Draft(): Response
    {
        $this->authorizeDeployHook();
        ...
        try {
            Artisan::call('db:seed', [
                '--class' => 'Database\\Seeders\\Article113Seeder',
                '--force' => true,
            ]);
        } catch (\Throwable $e) {
            report($e);

            return response('Article 113 draft seed failed: '.$e->getMessage(), 500);
        }

        $slug = 'fullstack-iot-device-id-dua-stasiun';
        $article = Article::where('slug', $slug)->first();
        if (! $article || $article->status !== 'draft' || $article->published_at !== null || Article::published()->where('slug', $slug)->exists()) {
            return response('Article 113 must remain draft (pre-launch B)', 500);
        }

        $requiredId = ['#113 (ini)', 'esp32-meja-02', 'stasiun.db', '5 baris meja-02 siap.', 'FS-44'];
        $missingId = array_values(array_filter($requiredId, fn (string $needle): bool => ! str_contains((string) $article->body, $needle)));
        if ($missingId !== []) {
            return response('Article 113 ID content checks failed: '.implode(', ', $missingId), 500);
        }

        $requiredEn = ['#113 (this article)', 'esp32-meja-02', 'stasiun.db', '5 baris meja-02 siap.', 'FS-44'];
        $missingEn = array_values(array_filter($requiredEn, fn (string $needle): bool => ! str_contains((string) $article->body_en, $needle)));
        if (! filled($article->title_en) || ! filled($article->body_en) || ! filled($article->seo_title_en) || ! filled($article->seo_description_en) || $missingEn !== []) {
            return response('Article 113 EN content checks failed: '.implode(', ', $missingEn), 500);
        }

        if (! str_contains((string) $article->cover_image, 'fs43-cover-device')) {
            return response('Article 113 cover check failed', 500);
        }

        Artisan::call('view:clear');

        return response('Article 113 seeded as draft (pre-launch B)', 200);
    }
