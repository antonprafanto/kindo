<section class="mt-10 pt-8 border-t-2 border-black" id="komentar">
    <h2 class="text-xl font-black mb-2 theme-heading">{{ __('ui.comments.heading') }}</h2>
    <p class="text-sm theme-muted mb-6">
        {{ __('ui.comments.intro') }}
    </p>
    ...
    @else
    <p class="text-sm theme-muted mb-6 italic">{{ __('ui.comments.empty') }}</p>
    @endif
    ...
    <div class="p-5 sm:p-6 border-2 border-black theme-paper" style="box-shadow: 4px 4px 0 #000;">
        ...
        <div class="mb-4 flex items-center justify-between gap-2 p-3 border-2 border-black text-sm theme-highlight">
            <span class="font-semibold">{{ __('ui.comments.replying') }}</span>
            <button type="button" wire:click="cancelReply" class="text-xs font-bold underline">{{ __('ui.comments.cancel') }}</button>
        </div>
        @endif
        ...
        <form wire:submit="submit" class="space-y-4">
            ...
            <div class="grid sm:grid-cols-2 gap-4">
                <div>
                    <label for="comment-author-name" class="block text-xs font-bold uppercase tracking-wider mb-2 theme-heading">{{ __('ui.comments.name') }}</label>
                    <input type="text" id="comment-author-name" wire:model="author_name" required maxlength="100"
                           ...
                           placeholder="{{ __('ui.comments.name_placeholder') }}"
                ...
                <div>
                    <label for="comment-author-email" class="block text-xs font-bold uppercase tracking-wider mb-2 theme-heading">{{ __('ui.comments.email') }}</label>
                    <input type="email" id="comment-author-email" wire:model="author_email" required maxlength="200"
                           ...
                           placeholder="{{ __('ui.comments.email_placeholder') }}"
            ...
            <div>
                <label for="comment-body" class="block text-xs font-bold uppercase tracking-wider mb-2 theme-heading">{{ __('ui.comments.body_label') }}</label>
                <textarea id="comment-body" wire:model="body" required rows="4" maxlength="2000"
                          ...
                          placeholder="{{ __('ui.comments.body_placeholder') }}"
            ...
            <div wire:ignore id="comment-turnstile-wrap" @class(['hidden' => ! $turnstileRequested])>
                <p class="text-sm font-semibold mb-2 theme-heading">{{ __('ui.comments.security') }}</p>
            ...
            <button type="submit" class="btn-brutal btn-primary px-6 py-3 text-sm" wire:loading.attr="disabled">
                ...
                <span wire:loading.remove wire:target="submit">{{ __('ui.comments.submit_verify') }}</span>
                @else
                <span wire:loading.remove wire:target="submit">{{ __('ui.comments.submit') }}</span>
                @endif
                <span wire:loading wire:target="submit">{{ __('ui.comments.processing') }}</span>
            </button>

            <p class="text-xs theme-muted">
                {{ __('ui.comments.privacy_note') }}
                <a href="{{ route('privacy') }}" class="underline">{{ __('ui.footer.privacy') }}</a>
            </p>
        </form>
    </div>
</section>
