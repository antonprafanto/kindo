<x-layouts.app :title="__('ui.articles.index_title')">

    <x-breadcrumb :items="[['label' => __('ui.articles.breadcrumb')]]" />
    ...
    <div class="max-w-6xl mx-auto px-4 py-10">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
            ...
            <form method="GET" action="{{ route('articles.index') }}" class="flex items-center gap-2">
                ...
                @endif
                <label class="text-xs font-bold uppercase tracking-wider theme-muted">{{ __('ui.articles.sort_label') }}</label>
                <select name="sort" onchange="this.form.submit()"
                        class="input-brutal py-2 pl-3 pr-8 text-sm w-auto"
                        style="box-shadow: 2px 2px 0 #000;">
                    <option value="terbaru" {{ $sort === 'terbaru' ? 'selected' : '' }}>{{ __('ui.articles.sort_newest') }}</option>
                    <option value="populer" {{ $sort === 'populer' ? 'selected' : '' }}>{{ __('ui.articles.sort_popular') }}</option>
                </select>
            </form>
        </div>
        ...
        <div class="py-24 text-center">
            ...
            <h2 class="text-xl font-bold mb-2">{{ __('ui.articles.empty_title') }}</h2>
            <p class="theme-muted">{{ __('ui.articles.empty_body') }}</p>
            <a href="{{ route('articles.index') }}" class="btn-brutal btn-primary mt-6 px-8 py-3 text-sm inline-flex">{{ __('ui.articles.empty_cta') }}</a>
        </div>
        @endif
    </div>
