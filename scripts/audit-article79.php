...
check('labeled SVG helper caption', str_contains($id, 'Skema berlabel') && str_contains($en, 'Labeled schematic'));
check('labeled SVG helper title in SVG', str_contains($id, 'Skema berlabel — bantu baca foto (belum coding)</text>') && str_contains($en, 'Labeled schematic — helps read the photo (no code yet)</text>'));
check('photo + skema tip ID', str_contains($id, 'foto rangkaian</strong> + <strong>skema berlabel'));
check('photo + schematic tip EN', str_contains($en, 'circuit photo</strong> + <strong>labeled schematic'));
...
check('no abstract SVG as main wiring title', ! str_contains($id, 'Gambar utama — LED menyala dari 3V3 (belum coding)</text>'));
