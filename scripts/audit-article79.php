...
check('label wiring SVG helper', str_contains($id, 'fs09-led-breadboard-wiring.svg') && str_contains($en, 'fs09-led-breadboard-wiring.svg') && is_file(__DIR__.'/../public/images/fsiot/fs09-led-breadboard-wiring.svg'));
check('label SVG caption ID', str_contains($id, 'Skema berlabel (bantu baca foto)') && str_contains($id, '1–7'));
check('label SVG caption EN', str_contains($en, 'Labeled schematic (helps read the photo)') && str_contains($en, '1–7'));
check('numbered points in wiring ID', str_contains($id, '1 + 2 — Power rail') && str_contains($id, '4 — Resistor'));
check('numbered points in wiring EN', str_contains($en, '1 + 2 — Power rails') && str_contains($en, '4 — 220Ω'));
check('short warning in main diagram', str_contains($id, 'Jangan sambungkan 3V3 dan GND') && str_contains($en, 'Never put 3V3 and GND'));
check('photo matches columns 2 and 7', str_contains($id, 'kolom 7') && str_contains($en, 'column 7') && str_contains($id, '220Ω') && str_contains($en, '220Ω'));
check('photo resistor is 220 not 1k', str_contains($id, 'Resistor di foto = <strong>220Ω</strong>') && str_contains($en, 'Resistor in the photo = <strong>220Ω</strong>') && ! str_contains($id, '1 kΩ') && ! str_contains($en, '1 kΩ'));
