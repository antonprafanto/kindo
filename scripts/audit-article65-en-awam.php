...
check('php demo file syntax test', str_contains($en, 'php laravel_pagination_filter_pencarian_demo.php'));
check('curl.exe Windows', str_contains($en, 'curl.exe'));
check('paginate mentioned', str_contains($en, 'paginate'));
check('#65 this article', str_contains($en, '#65 (this article)'));
check('filter + search q', (str_contains($en, 'status') || str_contains($en, 'filter')) && (str_contains($en, '?q=') || str_contains($en, 'q=')));
check('Seeder title_en + body_en', str_contains($src, "'title_en'") && str_contains($src, "'body_en'"));
check('Soft bridge Policy', str_contains($en, 'Authorization Policy') || str_contains($en, 'Policy'));
