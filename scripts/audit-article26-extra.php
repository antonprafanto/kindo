...
/**
 * Extra exhaustive audit #26 — regresi & edge case.
 */
...
echo "=== EXTRA EXHAUSTIVE AUDIT #26 ===\n\n";

echo "--- A: Regresi audit artikel terkait ---\n\n";

$relatedAudits = [
    'audit-article25.php' => '#25',
...
echo "\n--- B: Semua link internal #26 valid di DB ---\n\n";
...
check($pos26 !== false && $pos25 !== false, 'publishArticle26 + publishArticle25 ada');
...
if (preg_match('/function publishArticle26\(\)[^{]*\{([\s\S]*?)\n    \}/', $deploy, $m)) {
    foreach ($reseedClasses as $class) {
        check(str_contains($m[1], $class), "publishArticle26 re-seed {$class}");
    }
} else {
    check(false, 'publishArticle26() ditemukan di DeployController');
}
...
check(str_contains($yml, 'lora-esp32-modul-sx1278-kirim-data-jarak-jauh'), 'CI verifikasi slug #26');

echo "\n--- D: Konsistensi konten ---\n\n";

check(str_contains($body, 'mosquitto_sub'), 'Perintah mosquitto_sub uji gateway');
check(str_contains($body, 'LoRa.parsePacket'), 'API LoRa receiver');
check(str_contains($body, 'SPI.begin'), 'Init SPI');
check(str_contains($body, 'SDPPI'), 'Regulasi SDPPI');
check(str_contains($body, '433 MHz'), 'Band 433 MHz dijelaskan');
