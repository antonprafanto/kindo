...
/**
 * Ultra-deep supplemental audit #27 — cek di luar 5 skrip utama.
 * Usage: php scripts/audit-article27-ultra.php
 */
...
$seriesChecks = [
    ...
    'GANTI_NAMA_WIFI'                  => 'Placeholder WiFi SSID',
];
...
check(str_contains($body, 'MJPEG') && str_contains($body, 'ESP32-CAM'), 'Konsep visual ESP32-CAM + MJPEG');
...
foreach ([
    'Article26Seeder', 'Article20Seeder', 'Article10Seeder', 'Article6Seeder',
] as $cls) {
...
foreach ($inboundExpected as $sourceSlug => $label) {
    ...
    check(str_contains($srcBody, $href), "Inbound {$label} → #27 hyperlink");
}

echo "\n--- 5: Regresi #26 setelah re-seed ---\n\n";

$a26body = Article::where('slug', 'lora-esp32-modul-sx1278-kirim-data-jarak-jauh')->value('body') ?? '';
check(str_contains($a26body, $href), '#26 Langkah Selanjutnya hyperlink ke #27');
check(! str_contains($a26body, 'ESP32-CAM (#27):'), '#26 tidak ada teaser orphan ESP32-CAM (#27) tanpa link');
...
check(str_contains($deploy, 'function publishArticle27'), 'publishArticle27() ada');
check(str_contains($deploy, 'SitemapService'), 'Sitemap refresh di hook');

$yml = file_get_contents(__DIR__ . '/../.github/workflows/deploy.yml');
check(str_contains($yml, 'Publish article 27 via deploy hook (required)'), 'CI step #27 required');
check(str_contains($yml, 'esp32-cam-streaming-mjpeg-capture-foto-wifi'), 'CI verifikasi URL #27');
check(strpos($yml, 'publish-article-26') < strpos($yml, 'publish-article-27'), 'CI #26 sebelum #27');
