...
/**
 * Spot-check: body #49 lolos sanitizer tanpa kehilangan SVG/Pola Dasar.
 * Usage: php scripts/audit-article49-sanitize.php
 */
...
check(str_contains($out, 'oop49Arrow') || str_contains($out, 'marker-end'), 'SVG panah / marker id');
...
check(str_contains($out, '(#48)') || str_contains($out, '(#40)'), 'Anchor prasyarat');
...
echo "\n=== Sanitize spot-check #49: {$passed} passed, {$failed} failed ===\n";
exit($failed > 0 ? 1 : 0);
