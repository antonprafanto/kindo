...
/**
 * Spot-check: body #45 lolos sanitizer tanpa kehilangan SVG/Pola Dasar.
 * Usage: php scripts/audit-article45-sanitize.php
 */
...
check(str_contains($out, 'oop45Arrow') || str_contains($out, 'marker-end'), 'SVG panah / marker id');
...
check(str_contains($out, '(#44)'), 'Anchor #44');
...
echo "\n=== Sanitize spot-check #45: {$passed} passed, {$failed} failed ===\n";
exit($failed > 0 ? 1 : 0);
