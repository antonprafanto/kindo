...
/**
 * Sanitize spot-check #68.
 * Usage: php scripts/audit-article68-sanitize.php
 */
...
check(str_contains($body, 'assertJson') || str_contains($body, 'Feature Test'), 'Feature Test / assertJson');
check(substr_count($body, '#F5F5F0') >= 2, '≥2 figure #F5F5F0');
check(strlen($body) > 4000, 'Body tidak terpotong pendek');

echo "\n=== Sanitize spot-check #68: {$passed} passed, {$failed} failed ===\n";
exit($failed > 0 ? 1 : 0);
