...
/**
 * Spot-check: body #50 lolos sanitizer tanpa kehilangan SVG/Pola Dasar.
 * Usage: php scripts/audit-article50-sanitize.php
 */
...
check(str_contains($out, 'oop50Arrow') || str_contains($out, 'marker-end'), 'SVG panah / marker id');
...
check(str_contains($out, '(#49)') || str_contains($out, '(#45)'), 'Anchor prasyarat');
...
echo "\n=== Sanitize spot-check #50: {$passed} passed, {$failed} failed ===\n";
exit($failed > 0 ? 1 : 0);
