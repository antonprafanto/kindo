...
/**
 * Deep audit artikel #25 — ESP-NOW.
 * Usage: php scripts/audit-article25-deep.php
 */
...
check(substr_count($body, '<table>') >= 1, 'Minimal 1 tabel');
...
foreach ($backlinkSources as $sourceSlug => $label) {
    ...
    check(str_contains($src?->body ?? '', $slug), "Backlink {$label} → #25");
}
...
check(str_contains($body, 'esp_now_add_peer'), 'Peer registration API');
check(str_contains($body, 'sensor_packet_t'), 'Struct data bersama');
