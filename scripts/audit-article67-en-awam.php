...
check('notepad Resource path', str_contains($en, 'notepad app\Http\Resources\PeminjamanResource.php'));
check('notepad controller path', str_contains($en, 'notepad app\Http\Controllers\PeminjamanController.php'));
...
check('php demo file syntax test', str_contains($en, 'php laravel_api_resource_json_demo.php'));
check('curl.exe Windows', str_contains($en, 'curl.exe'));
check('JsonResource mentioned', str_contains($en, 'JsonResource'));
check('PeminjamanResource mentioned', str_contains($en, 'PeminjamanResource'));
check('#67 this article', str_contains($en, '#67 (this article)'));
check('Seeder title_en + body_en', str_contains($src, "'title_en'") && str_contains($src, "'body_en'"));
check('Soft bridge Feature Test', str_contains($en, 'Feature Test') && ! str_contains($en, '/artikel/laravel-feature-test-api'));
