<?php
/** Quick final verification — run: php scripts/audit-article25-final.php */
...
c($a && $a->status === 'published', 'Artikel #25 published di DB');
...
foreach ($sources as $s => $lbl) {
    ...
    c(str_contains($b, $href), "$lbl → #25 hyperlink");
}

$r = app()->handle(Illuminate\Http\Request::create('/artikel/' . $slug, 'GET'));
$html = $r->getContent();
c($r->getStatusCode() === 200, 'HTTP 200 lokal');
c(str_contains($html, 'ESP-NOW') && str_contains($html, 'esp_now_init'), 'Konten kunci ter-render');

$deploy = file_get_contents(__DIR__ . '/../app/Http/Controllers/DeployController.php');
if (preg_match('/function publishArticle25\(\)[^{]*\{([\s\S]*?)\n    \}/', $deploy, $m)) {
    $hookBody = $m[1];
    c(strpos($hookBody, 'Article25Seeder') < strpos($hookBody, 'Article20Seeder'), 'Hook: seed #25 sebelum re-seed backlink');
    c(strpos($hookBody, 'Article7Seeder') < strpos($hookBody, 'runDuplicateBme280Cleanup'), 'Hook: re-seed #7 sebelum cleanup');
} else {
    c(false, 'publishArticle25() ditemukan di DeployController');
}

$yml = file_get_contents(__DIR__ . '/../.github/workflows/deploy.yml');
c(strpos($yml, 'publish-article-20') < strpos($yml, 'publish-article-25'), 'CI: hook #20 sebelum #25');
c(preg_match('/Publish article 25 via deploy hook \(required\)/', $yml) === 1, 'CI: hook #25 required');
