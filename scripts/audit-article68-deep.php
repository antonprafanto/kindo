...
/**
 * Deep-audit pass-1 #68 — Feature Test API ramah awam.
 * Usage: php scripts/audit-article68-deep.php
 */
...
check(str_contains($body, 'assertJson') && str_contains($body, 'assertStatus') && str_contains($body, 'Feature Test'), 'assertJson + assertStatus + Feature Test');
check(str_contains($body, 'Kenapa PHP') || str_contains($body, 'PHP biasa'), 'Fondasi PHP dulu');
check(str_contains($body, 'Seri 5') && str_contains($body, '#68 (ini)'), 'Framing + self-ref');
check(str_contains($body, 'Laravel 13+'), 'Pin Laravel');
check(substr_count($body, '/artikel/laravel-api-resource-json') >= 1, '≥1 link #67');
check(! preg_match('/(?<![\w\/"#>])#68(?!\s*\(ini\))/', strip_tags(preg_replace('/<a\b[^>]*>.*?<\/a>/is', '', $body) ?? '')), 'Tidak bare #68 (kecuali ini)');
...
check(str_contains($body, 'laravel_feature_test_api_demo.php') && str_contains($body, 'demo('), 'File + demo');
...
check(str_contains($src, 'laravel-feature-test-api'), 'Slug');
check(str_contains($src, "'title_en'") && str_contains($src, "'body_en'"), 'Field EN');
check(str_contains($bodyEn, '#68 (this article)') && str_contains($bodyEn, 'Beginner:'), 'EN body dasar');
...
check(str_contains(file_get_contents(__DIR__.'/../app/Http/Controllers/DeployController.php'), 'publishArticle68'), 'Hook');
check(str_contains($deployYml, 'publish-article-68'), 'CI step #68 ada');
check(! preg_match('/Publish article 68 via deploy hook \(WIP\)\s*\n\s*continue-on-error:\s*true/u', $deployYml) && str_contains($deployYml, 'Publish article 68 via deploy hook (required)'), 'CI #68 required');
...
check(str_contains($body, 'Rate Limiting') && ! str_contains($body, '/artikel/laravel-rate-limiting-api'), 'Jembatan soft ke Rate Limiting');
check(str_contains($body, 'slip pinjam') || str_contains($body, 'checklist'), 'Analogi loket');
...
echo "\n=== Deep-audit pass-1 #68: {$passed} passed, {$failed} failed ===\n";
if ($failed === 0) {
    echo "Verdict: #68 feature-test ID+EN tools-first hijau — siap LIVE.\n";
}
exit($failed > 0 ? 1 : 0);
