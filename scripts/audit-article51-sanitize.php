...
/**
 * Spot-check sanitizer #51.
 * Usage: php scripts/audit-article51-sanitize.php
 */
...
check(str_contains($out, 'oop51Arrow') || str_contains($out, 'marker-end'), 'SVG panah / marker id');
...
check(str_contains($out, '(#50)') || str_contains($out, '(#47)'), 'Anchor prasyarat');
...
echo "\n=== Sanitize spot-check #51: {$passed} passed, {$failed} failed ===\n";
exit($failed > 0 ? 1 : 0);
