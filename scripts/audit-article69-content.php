...
/**
 * Content / checklist audit #69 Rate Limiting API.
 * Usage: php scripts/audit-article69-content.php
 */
...
echo "=== Content / checklist audit #69 ===\n\n";
...
check(str_contains($body, '#69 (ini)'), 'Self-ref #69 (ini)');
check(! preg_match('/(?<![\w\/"#>])#70(?!\s*\(ini\))/', strip_tags(preg_replace('/<a\b[^>]*>.*?<\/a>/is', '', $body) ?? '')), 'Tidak plain #70');
...
check(str_contains($body, 'cara menguji bagian ini') && str_contains($body, 'laravel_rate_limiting_api_demo.php'), 'Cara uji demo lengkap');
...
check(str_contains(file_get_contents(__DIR__.'/../routes/web.php'), 'publish-article-69'), 'Route hook');
...
check(file_exists(__DIR__.'/audit-article69.php'), 'Audit utama ada');
check(file_exists(__DIR__.'/audit-article69-php.php'), 'Audit PHP ada');
check(file_exists(__DIR__.'/audit-article69-sanitize.php'), 'Audit sanitize ada');
check(file_exists(__DIR__.'/audit-article69-deep.php'), 'Deep pass-1 ada');
check(str_contains($body, 'PHP biasa') || str_contains($body, 'Kenapa PHP'), 'Narasi PHP dulu');
check(str_contains($body, 'loket') || str_contains($body, 'antrean'), 'Analogi loket/antrean');
check(str_contains($body, 'RateLimiter') && str_contains($body, 'throttle'), 'Gloss RateLimiter/throttle');
...
check(str_contains($bodyEn, '#69 (this article)') && str_contains($bodyEn, 'Beginner:'), 'Body EN ada');
...
check(str_contains($bodyEn, 'how to test this part') && str_contains($bodyEn, 'laravel_rate_limiting_api_demo.php'), 'EN cara uji demo');
check(str_contains($body, 'php batas-cek.php') && str_contains($body, 'demo(') && str_contains($body, 'curl.exe'), '3-tier batas-cek → demo → curl spam');
