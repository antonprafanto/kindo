...
/**
 * Spot-check: body #48 lolos sanitizer tanpa kehilangan SVG/Pola Dasar.
 * Usage: php scripts/audit-article48-sanitize.php
 */
...
check(str_contains($out, 'oop48Arrow') || str_contains($out, 'marker-end'), 'SVG panah / marker id');
...
check(str_contains($out, '(#47)') || str_contains($out, '(#42)'), 'Anchor prasyarat');
...
echo "\n=== Sanitize spot-check #48: {$passed} passed, {$failed} failed ===\n";
exit($failed > 0 ? 1 : 0);
