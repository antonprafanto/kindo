...
/**
 * Extra exhaustive audit #18 — cek regresi & edge case di luar 3 skrip utama.
 */
...
echo "=== EXTRA EXHAUSTIVE AUDIT #18 ===\n\n";

echo "--- A: Regresi audit artikel terkait (teaser #18) ---\n\n";

$relatedAudits = [
    'audit-article16.php' => '#16',
    'audit-article17.php' => '#17',
    'audit-article23.php' => '#23',
    'audit-article24.php' => '#24',
...
echo "\n--- B: Semua link internal #18 valid di DB ---\n\n";
...
check(count($uniqueLinks) >= 6, 'Minimal 6 link internal unik (' . count($uniqueLinks) . ')');
...
$sources = [
    ...
    'broker-mosquitto-pribadi-raspberry-pi-vps-autentikasi-esp32' => ['#16', 2],
...
foreach ($sources as $sourceSlug => [$label, $minCount]) {
    ...
    check($count >= $minCount, "{$label} punya ≥{$minCount} hyperlink ke #18 (ada {$count})");
}
...
$checks = [
    ...
    'kindo_esp32'                      => 'User publisher ESP32',
    'kindo_iot'                        => 'User MySQL',
    '192.168.1.50'                     => 'IP broker contoh',
    'GANTI_PASSWORD_SUBSCRIBER'        => 'Placeholder subscriber',
    'GANTI_PASSWORD_MYSQL'             => 'Placeholder MySQL',
    'GANTI_PASSWORD_MQTT'              => 'Placeholder publisher',
];
...
$seederSource = file_get_contents(__DIR__ . '/../database/seeders/Article18Seeder.php');
...
$pos18 = strpos($deploy, 'Article18Seeder');
$pos34 = strpos($deploy, "Article34Seeder',\n            '--force' => true,\n        ]);\n\n        Artisan::call('db:seed', [\n            '--class' => 'Database\\\\Seeders\\\\Article17Seeder'");
check($pos18 !== false && $pos34 !== false && $pos18 < $pos34, 'Article18Seeder di-seed sebelum re-seed backlink');

$reseedClasses = [
    'Article34Seeder', 'Article17Seeder', 'Article16Seeder', 'Article10Seeder',
    'Article7Seeder', 'Article13Seeder', 'Article14Seeder', 'Article23Seeder', 'Article24Seeder',
];
foreach ($reseedClasses as $class) {
    check(str_contains($deploy, $class), "publishArticle18 re-seed {$class}");
}
...
check(str_contains($html, 'parse_waktu'), 'Fungsi parse_waktu ter-render');
check(str_contains($html, 'kindo-mqtt-mysql.service'), 'systemd unit ter-render');
...
echo "\n--- J: #34 masih ≥4 link ke #18 setelah seed ---\n\n";

$a34 = Article::where('slug', 'ntp-timestamp-esp32-waktu-akurat-log-sensor-mqtt')->first();
check(substr_count($a34?->body ?? '', $href) >= 4, '#34 punya ≥4 hyperlink ke #18');
