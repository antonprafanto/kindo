...
/**
 * Sanitize spot-check #65.
 * Usage: php scripts/audit-article65-sanitize.php
 */
...
check(str_contains($body, 'paginate') || str_contains($body, 'array_slice'), 'Pagination');
check(substr_count($body, '#F5F5F0') >= 2, '≥2 figure #F5F5F0');
check(strlen($body) > 4000, 'Body tidak terpotong pendek');

echo "\n=== Sanitize spot-check #65: {$passed} passed, {$failed} failed ===\n";
exit($failed > 0 ? 1 : 0);
