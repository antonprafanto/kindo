...
check('notepad policy path', str_contains($en, 'notepad app\Policies\PeminjamanPolicy.php') || str_contains($en, 'app\Policies\PeminjamanPolicy'));
...
check('php demo file syntax test', str_contains($en, 'php laravel_policy_otorisasi_api_demo.php'));
check('curl.exe Windows', str_contains($en, 'curl.exe'));
check('authorize mentioned', str_contains($en, 'authorize'));
check('PeminjamanPolicy mentioned', str_contains($en, 'PeminjamanPolicy'));
check('#66 this article', str_contains($en, '#66 (this article)'));
check('Seeder title_en + body_en', str_contains($src, "'title_en'") && str_contains($src, "'body_en'"));
check('Soft bridge API Resource', str_contains($en, 'API Resource') && ! str_contains($en, '/artikel/laravel-api-resource-json'));
