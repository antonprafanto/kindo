...
/**
 * Sanitize spot-check #57.
 * Usage: php scripts/audit-article57-sanitize.php
 */
...
check(str_contains($body, 'validate') || str_contains($body, 'FormRequest'), 'Validate/FormRequest');
check(substr_count($body, 'background:#F5F5F0') >= 2, '≥2 figure #F5F5F0');
check(strlen($body) > 4000, 'Body tidak terpotong pendek');

echo "\n=== Sanitize spot-check #57: {$passed} passed, {$failed} failed ===\n";
exit($failed > 0 ? 1 : 0);
