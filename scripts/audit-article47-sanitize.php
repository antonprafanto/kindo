...
/**
 * Spot-check: body #47 lolos sanitizer tanpa kehilangan SVG/Pola Dasar.
 * Usage: php scripts/audit-article47-sanitize.php
 */
...
check(str_contains($out, 'oop47Arrow') || str_contains($out, 'marker-end'), 'SVG panah / marker id');
...
check(str_contains($out, '(#46)') || str_contains($out, '(#44)'), 'Anchor prasyarat');
...
echo "\n=== Sanitize spot-check #47: {$passed} passed, {$failed} failed ===\n";
exit($failed > 0 ? 1 : 0);
