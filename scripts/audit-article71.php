...
check('no Awam stamp ID', ! str_contains($id, 'Awam:') && ! str_contains($id, 'Awam —') && ! str_contains($id, '<strong>Awam</strong>'));
check('no Beginner stamp EN', ! str_contains($en, 'Beginner:') && ! str_contains($en, 'Beginner —'));
check('no awam reader stamp in body ID', ! preg_match('/\bawam\b/i', $id));
check('no beginner reader stamp in body EN', ! preg_match('/\bbeginner\b/i', $en));
check('friendly tip labels ID', str_contains($id, 'Intinya:') && str_contains($id, 'Analogi:') && str_contains($id, 'Cara pakai artikel ini'));
check('friendly tip labels EN', str_contains($en, 'In short:') && str_contains($en, 'Analogy:') && str_contains($en, 'How to use this article'));
...
check('ID board legend table', str_contains($id, 'Arti sederhana') && str_contains($id, 'USB-to-UART Bridge') && str_contains($id, 'Micro-USB atau USB-C'));
check('EN board legend table', str_contains($en, 'Simple meaning') && str_contains($en, 'Micro-USB or USB-C'));
...
check('station SVG dashboard label', str_contains($id, 'dashboard'));
...
foreach (['Pendahuluan', 'Persiapan', 'Kesalahan yang sering terjadi', 'Kesimpulan', 'Awam:', 'Intinya:'] as $w) {
    if ($w === 'Intinya:') {
        check('No residual Indo tip label in EN: Intinya', ! str_contains($en, 'Intinya:'));
        continue;
    }
