...
/**
 * Spot-check sanitizer #52.
 * Usage: php scripts/audit-article52-sanitize.php
 */
...
check(str_contains($out, 'oop52Arrow') || str_contains($out, 'marker-end'), 'SVG panah / marker id');
...
check(str_contains($out, '(#51)') || str_contains($out, '(#49)'), 'Anchor prasyarat');
...
echo "\n=== Sanitize spot-check #52: {$passed} passed, {$failed} failed ===\n";
exit($failed > 0 ? 1 : 0);
