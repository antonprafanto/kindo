...
check104('MQTTX JSON state describes Host/JSON without saying error', str_contains($body, 'MQTTX sudah menampilkan JSON dari ESP32') && str_contains($bodyEn, 'MQTTX is already showing JSON from the ESP32') && ! str_contains($body, 'bukan layar error') && ! str_contains($bodyEn, 'not an error screen'));
