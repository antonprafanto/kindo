...
check('helper labeled SVG file', str_contains($id, 'fs10-button-pulldown-wiring.svg') && str_contains($en, 'fs10-button-pulldown-wiring.svg') && is_file(__DIR__.'/../public/images/fsiot/fs10-button-pulldown-wiring.svg'));
$svgPath = __DIR__.'/../public/images/fsiot/fs10-button-pulldown-wiring.svg';
$svgBytes = is_file($svgPath) ? file_get_contents($svgPath) : '';
check('wiring SVG is valid UTF-8', $svgBytes !== '' && mb_check_encoding($svgBytes, 'UTF-8'));
check('wiring SVG ASCII-safe Ohm', str_contains($svgBytes, 'kOhm') && ! str_contains($svgBytes, "\x97"));
...
check('wiring labeled legs ID', str_contains($id, 'kaki A') && str_contains($id, 'Ke GND') && str_contains($id, 'titik sinyal (S)'));
check('wiring labeled legs EN', str_contains($en, 'leg A') && str_contains($en, 'To GND') && str_contains($en, 'signal node (S)'));
...
check('helper schematic caption ID', str_contains($id, 'Skema berlabel'));
check('helper schematic caption EN', str_contains($en, 'Labeled schematic'));
