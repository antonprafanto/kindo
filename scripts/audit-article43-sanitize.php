...
/**
 * Spot-check: body #43 lolos sanitizer tanpa kehilangan SVG/Pola Dasar.
 * Usage: php scripts/audit-article43-sanitize.php
 */
...
check(str_contains($out, 'oop43Arrow') || str_contains($out, 'marker-end'), 'SVG panah / marker id');
...
check(str_contains($out, '(#42)'), 'Anchor #42');
...
echo "\n=== Sanitize spot-check #43: {$passed} passed, {$failed} failed ===\n";
exit($failed > 0 ? 1 : 0);
