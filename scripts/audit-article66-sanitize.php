...
/**
 * Sanitize spot-check #66.
 * Usage: php scripts/audit-article66-sanitize.php
 */
...
check(str_contains($body, 'authorize') || str_contains($body, 'PeminjamanPolicy'), 'Policy');
check(substr_count($body, '#F5F5F0') >= 2, '≥2 figure #F5F5F0');
check(strlen($body) > 4000, 'Body tidak terpotong pendek');

echo "\n=== Sanitize spot-check #66: {$passed} passed, {$failed} failed ===\n";
exit($failed > 0 ? 1 : 0);
