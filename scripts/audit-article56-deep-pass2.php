...
check(str_contains($body55, '8/8 Capstone Laravel selesai'), '#55 Progress 5/8');
...
check(str_contains($deploy, 'laravel56jsonArrow') && str_contains($deploy, '8/8 Capstone Laravel selesai') && str_contains($deploy, 'Developer Tools') && str_contains($deploy, 'merapikan daftar'), 'Hook body locks');
...
check(str_contains($body, 'laravel-request-validasi-api'), '#56 hardlink slug #57');
