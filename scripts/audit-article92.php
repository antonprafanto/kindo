<?php

/**
 * Local audit for Article92Seeder (FS-22) — pre-launch draft.
 * Run: php scripts/audit-article92.php
 */

require __DIR__.'/../vendor/autoload.php';
$app = require __DIR__.'/../bootstrap/app.php';
$app->make(Illuminate\Contracts\Console\Kernel::class)->bootstrap();

use Database\Seeders\Article92Seeder;

$ref = new ReflectionClass(Article92Seeder::class);
$idM = $ref->getMethod('body');
$idM->setAccessible(true);
$enM = $ref->getMethod('bodyEn');
$enM->setAccessible(true);
$seeder = $ref->newInstanceWithoutConstructor();
$id = $idM->invoke($seeder);
$en = $enM->invoke($seeder);
$src = file_get_contents(__DIR__.'/../database/seeders/Article92Seeder.php');
$routes = file_get_contents(__DIR__.'/../routes/web.php');
$show = file_get_contents(__DIR__.'/../resources/views/articles/show.blade.php');

$pass = 0;
$fail = 0;

function check(string $label, bool $ok): void
{
    global $pass, $fail;
    echo ($ok ? 'OK    ' : 'FAIL  ').$label.PHP_EOL;
    $ok ? $pass++ : $fail++;
}

check('status draft in seeder', str_contains($src, "'status'") && str_contains($src, "'draft'"));
check('published_at null', str_contains($src, "'published_at'") && str_contains($src, 'null'));
check('slug fullstack-iot-ldr-adc-seberapa-terang', str_contains($src, 'fullstack-iot-ldr-adc-seberapa-terang'));
check('category iot-smart-device', str_contains($src, 'iot-smart-device'));
check('tag fullstack-iot', str_contains($src, "'fullstack-iot'"));
check('title_en + body_en', str_contains($src, "'title_en'") && str_contains($src, "'body_en'"));
check('seo without Awam/Beginner', ! str_contains($src, 'Awam') && ! str_contains($src, 'Beginner for'));
check('no publish hook yet', ! str_contains($routes, 'publish-article-92'));
check('seed route exists', str_contains($routes, 'seed-article-92-draft'));

check('ID self-ref #92 (ini)', str_contains($id, '#92 (ini)'));
check('EN self-ref #92 (this article)', str_contains($en, '#92 (this article)'));
check('no Awam stamp ID', ! str_contains($id, 'Awam:') && ! str_contains($id, 'Awam —'));
check('no Beginner stamp EN', ! str_contains($en, 'Beginner:') && ! str_contains($en, 'Beginner —'));
check('no awam word in body ID', ! preg_match('/\bawam\b/i', $id));
check('no beginner word in body EN', ! preg_match('/\bbeginner\b/i', $en));
check('friendly tip labels ID', str_contains($id, 'Intinya:') && str_contains($id, 'Cara pakai artikel ini') && str_contains($id, 'Analogi:'));
check('friendly tip labels EN', str_contains($en, 'In short:') && str_contains($en, 'How to use this article') && str_contains($en, 'Analogy:'));
check('H2 parity', substr_count($id, '<h2') === substr_count($en, '<h2'));
check('wiring PNG asset', str_contains($id, 'fs22-ldr-wiring.png') && is_file(__DIR__.'/../public/images/fsiot/fs22-ldr-wiring.png'));
check('breadboard PNG asset', str_contains($id, 'fs22-ldr-breadboard.png') && is_file(__DIR__.'/../public/images/fsiot/fs22-ldr-breadboard.png'));
check('principle PNG asset', str_contains($id, 'fs22-ldr-principle.png') && is_file(__DIR__.'/../public/images/fsiot/fs22-ldr-principle.png'));
check('Gambar utama label ID', str_contains($id, 'Gambar utama'));
check('Main figure label EN', str_contains($en, 'Main figure'));
check('Skema bantu ID', str_contains($id, 'Skema bantu'));
check('Helper schematic EN', str_contains($en, 'Helper schematic'));
check('figures both >= 8', substr_count($id, '<figure') >= 8 && substr_count($en, '<figure') >= 8);
check('cover asset exists', is_file(__DIR__.'/../public/images/fsiot/fs22-cover-ldr.jpg'));
check('cover set-if-blank in seeder', str_contains($src, 'fs22-cover-ldr.jpg') && str_contains($src, 'cover_image'));
check('kit-ldr asset', str_contains($id, 'kit-ldr.jpg') && is_file(__DIR__.'/../public/images/fsiot/kit-ldr.jpg'));
check('10kohm asset', str_contains($id, 'kit-resistor-10kohm.jpg') && is_file(__DIR__.'/../public/images/fsiot/kit-resistor-10kohm.jpg'));

check('ID mistakes heading', str_contains($id, 'Kesalahan yang sering terjadi'));
check('EN mistakes heading', str_contains($en, 'Common mistakes'));
check('tools-first Upload IDE', str_contains($id, 'Tidak perlu hari ini') && str_contains($en, 'Not needed today') && str_contains($id, 'Arduino IDE'));
check('how to test IDE Upload ID', str_contains($id, 'Cara menguji perintah di atas') && str_contains($id, 'Arduino IDE'));
check('how to test IDE Upload EN', str_contains($en, 'How to test the commands above') && str_contains($en, 'Arduino IDE'));

check('IDE asset', str_contains($id, 'fs11-ide-overview-cite.png') && is_file(__DIR__.'/../public/images/fsiot/fs11-ide-overview-cite.png'));
check('board overview', str_contains($id, 'esp32-devkitc-overview.jpg') && is_file(__DIR__.'/../public/images/fsiot/esp32-devkitc-overview.jpg'));
check('Commons LDR cite', str_contains($id, 'commons.wikimedia.org') && str_contains($en, 'commons.wikimedia.org'));
check('KI diagram cite', str_contains($id, 'buatan Koding Indonesia') && str_contains($en, 'diagram by Koding Indonesia'));
check('Arduino analogRead cite', str_contains($id, 'analogRead') && str_contains($en, 'analogRead'));

check('sketch FS22_ldr_adc', str_contains($id, 'FS22_ldr_adc') && str_contains($en, 'FS22_ldr_adc'));
check('LDR', str_contains($id, 'LDR') && str_contains($en, 'LDR'));
check('GPIO 34', str_contains($id, 'GPIO 34') && str_contains($en, 'GPIO 34'));
check('10 kOhm required ID', str_contains($id, '10 kΩ') && str_contains($id, 'wajib'));
check('ADC 12-bit / 4095', str_contains($id, '4095') && str_contains($en, '4095'));
check('GELAP/REDUP/TERANG ID', str_contains($id, 'GELAP') && str_contains($id, 'REDUP') && str_contains($id, 'TERANG'));
check('DARK/DIM/BRIGHT EN', str_contains($en, 'DARK') && str_contains($en, 'DIM') && str_contains($en, 'BRIGHT'));
check('BUILDER soft via FS-18', str_contains($id, 'FS-18') && str_contains($en, 'FS-18'));
check('soft mention FS-17', str_contains($id, 'FS-17') && str_contains($en, 'FS-17'));
check('soft mention FS-21', str_contains($id, 'FS-21') && str_contains($en, 'FS-21'));

preg_match_all('/<svg[\s\S]*?<\/svg>/', $id, $svgIdBlocks);
preg_match_all('/<svg[\s\S]*?<\/svg>/', $en, $svgEnBlocks);
$noStrongInSvg = array_reduce($svgIdBlocks[0] ?? [], fn (bool $c, string $s): bool => $c && ! str_contains($s, '<strong>'), true)
    && array_reduce($svgEnBlocks[0] ?? [], fn (bool $c, string $s): bool => $c && ! str_contains($s, '<strong>'), true);
check('no strong tags inside SVG', $noStrongInSvg);
check('no tspan in SVG', ! str_contains($id, '<tspan') && ! str_contains($en, '<tspan'));

check('checklist ul id survives sanitizer', str_contains(app(\App\Services\ArticleHtmlSanitizer::class)->sanitize($id), 'id="fsiot-ldr-checklist-items"'));
check('checklist h2 id survives sanitizer', str_contains(app(\App\Services\ArticleHtmlSanitizer::class)->sanitize($id), 'id="fsiot-ldr-checklist"'));
check('checklist has 10 items ID', substr_count(strstr($id, 'fsiot-ldr-checklist-items'), '<li>') >= 10);
check('checklist has 10 items EN', substr_count(strstr($en, 'fsiot-ldr-checklist-items'), '<li>') >= 10);
check('interactive ldr checklist wired', str_contains($show, 'initFsiotLdrChecklist') && str_contains($show, 'fsiot-ldr-checklist'));
check('ldr checklist lang ID', str_contains(file_get_contents(__DIR__.'/../lang/id/ui.php'), 'fsiot_ldr_badge'));
check('ldr checklist lang EN', str_contains(file_get_contents(__DIR__.'/../lang/en/ui.php'), 'fsiot_ldr_badge'));

check('soft bridge FS-23', str_contains($id, 'FS-23') && str_contains($en, 'FS-23'));
check('jalur page link', str_contains($id, '/belajar/fullstack-iot') && str_contains($en, '/belajar/fullstack-iot'));
check('kesalahan >= 5', substr_count($id, '<li><strong>') >= 5);
check('no hardlink FS articles', ! preg_match('#/artikel/fullstack-iot-[a-z0-9-]+#', $id.$en));
check('escaped lt in code ID', str_contains($id, 'nilai &lt;') || str_contains($id, '&lt; AMBANG'));
check('escaped lt in code EN', str_contains($en, 'value &lt;') || str_contains($en, '&lt; THRESH'));

$enPlain = strip_tags($en);
foreach (['Pendahuluan', 'Persiapan', 'Kesalahan yang sering terjadi', 'Intinya:', 'Analogi:', 'Cara pakai'] as $indo) {
    check('No residual Indo in EN: '.$indo, ! str_contains($enPlain, $indo));
}

echo PHP_EOL."$pass pass / $fail fail".PHP_EOL;
exit($fail > 0 ? 1 : 0);
