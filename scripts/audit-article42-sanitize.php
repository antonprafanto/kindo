...
/**
 * Spot-check: body #42 lolos sanitizer tanpa kehilangan SVG/Pola Dasar.
 * Usage: php scripts/audit-article42-sanitize.php
 */
...
check(str_contains($out, 'oop42Arrow') || str_contains($out, 'marker-end'), 'SVG panah / marker id');
...
check(str_contains($out, '(#41)'), 'Anchor #41');
...
echo "\n=== Sanitize spot-check #42: {$passed} passed, {$failed} failed ===\n";
exit($failed > 0 ? 1 : 0);
