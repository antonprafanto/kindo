...
check($indexItems === 16, '#10 indeks punya 16 item live (' . $indexItems . ')');
check(str_contains($a10body, 'enam belas artikel'), '#10 teks enam belas artikel');
check(str_contains($a10body, $href), '#10 item #25 di indeks');
check(str_contains($a10body, 'LoRa') || str_contains($a10body, '#26'), '#10 teaser #26 LoRa di Masih akan datang');
...
if ($preDeploy) {
    ...
    check(str_contains($prod10html, 'lima belas artikel'), 'Production #10 masih lima belas — akan fix saat deploy hook #25');
} else {
    $prod20html = (string) shell_exec('curl -sS --max-time 20 ' . escapeshellarg('https://kodingindonesia.com/artikel/' . $slug20));
    $prod10html = (string) shell_exec('curl -sS --max-time 20 ' . escapeshellarg('https://kodingindonesia.com/artikel/dashboard-esp32-web-server-mqtt-monitoring-dht22'));
    check(str_contains($prod20html, $href), 'Production #20 hyperlink #25');
    check(str_contains($prod10html, 'enam belas artikel'), 'Production #10 enam belas artikel');
