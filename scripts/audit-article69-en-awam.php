...
check('notepad AppServiceProvider path', str_contains($en, 'notepad app\Providers\AppServiceProvider.php'));
...
check('php demo file syntax test', str_contains($en, 'php laravel_rate_limiting_api_demo.php'));
...
check('how to test demo', str_contains($en, 'how to test this part') && str_contains($en, 'laravel_rate_limiting_api_demo.php'));
check('RateLimiter mentioned', str_contains($en, 'RateLimiter'));
check('throttle mentioned', str_contains($en, 'throttle'));
check('429 mentioned', str_contains($en, '429'));
check('#69 this article', str_contains($en, '#69 (this article)'));
check('Seeder title_en + body_en', str_contains($src, "'title_en'") && str_contains($src, "'body_en'"));
check('Soft bridge Capstone', str_contains($en, 'Capstone') && ! str_contains($en, '/artikel/capstone-pinjam-kembali-laravel'));
...
$indoH2Hints = 0;
foreach ($enH2 as $t) {
    if (preg_match('/\b(Pendahuluan|Persiapan|Kesalahan|Kesimpulan|Istilah|Spesifikasi fitur|Alur batas)\b/u', $t)) {
        $indoH2Hints++;
