...
/**
 * Content / checklist audit #67 API Resource JSON.
 * Usage: php scripts/audit-article67-content.php
 */
...
echo "=== Content / checklist audit #67 ===\n\n";
...
check(str_contains($body, '#67 (ini)'), 'Self-ref #67 (ini)');
check(! preg_match('/(?<![\w\/"#>])#68(?!\s*\(ini\))/', strip_tags(preg_replace('/<a\b[^>]*>.*?<\/a>/is', '', $body) ?? '')), 'Tidak plain #68');
...
check(str_contains(file_get_contents(__DIR__.'/../routes/web.php'), 'publish-article-67'), 'Route hook');
...
check(str_contains($body, 'PeminjamanResource.php') || str_contains($body, 'app\\Http\\Resources\\PeminjamanResource'), 'Path Resource');
...
check(file_exists(__DIR__.'/audit-article67.php'), 'Audit utama ada');
check(file_exists(__DIR__.'/audit-article67-php.php'), 'Audit PHP ada');
check(file_exists(__DIR__.'/audit-article67-sanitize.php'), 'Audit sanitize ada');
check(file_exists(__DIR__.'/audit-article67-deep.php'), 'Deep pass-1 ada');
...
check(str_contains($body, 'toArray') && str_contains($body, 'status_label'), 'Gloss toArray/status_label');
...
check(str_contains($bodyEn, '#67 (this article)') && str_contains($bodyEn, 'Beginner:'), 'Body EN ada');
...
check(str_contains($body, 'php rapikan-cek.php') && str_contains($body, 'demo(') && str_contains($body, 'curl.exe'), '3-tier rapikan-cek → demo → curl');
