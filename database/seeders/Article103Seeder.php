...
class Article103Seeder extends Seeder
{
    ...
    {
        $tools = $this->figure('fs33-tools-order.png', 'Urutan tools: browser, pasang Mosquitto, PowerShell, lalu MQTTX', '<strong>Urutan meja kerja:</strong> browser → pemasang Mosquitto → PowerShell untuk menjalankan broker → MQTTX. Baru sekarang Connect boleh. Diagram buatan Koding Indonesia (FS-33).');
        $downloads = $this->figure('fs33-mosquitto-downloads.png', 'Halaman unduhan resmi Eclipse Mosquitto di mosquitto.org/download', '<strong>Ini halaman yang harus dibuka.</strong> Gulir ke bagian <strong>Windows</strong>, lalu unduh berkas <code>x64.exe</code> untuk komputer 64-bit. Jangan unduh berkas Source (.tar.gz) untuk lab ini. Sumber: <a href="https://mosquitto.org/download/" target="_blank" rel="noopener noreferrer">mosquitto.org/download</a> — Eclipse Mosquitto / Eclipse Foundation. Tangkapan layar 13 Agustus 2026. Mosquitto berlisensi Eclipse Public License 2.0 dan Eclipse Distribution License 1.0.');
        ...
        $mqttx = $this->figure('fs33-mqttx-local.png', 'Ilustrasi MQTTX tersambung ke Host 127.0.0.1 dan Port 1883', '<strong>Sekarang Connect boleh.</strong> Host = <code>127.0.0.1</code>, Port = <code>1883</code>. Jangan salin alamat broker dari screenshot internet. Ilustrasi buatan Koding Indonesia (FS-33), meniru tata letak aplikasi resmi <a href="https://mqttx.app/" target="_blank" rel="noopener noreferrer">MQTTX oleh EMQ</a> (Apache License 2.0). Screenshot jendela resmi tidak dipakai utuh karena menampilkan broker publik.');
...
HTML;
    ...
    {
        $tools = $this->figure('fs33-tools-order.png', 'Tool order: browser, install Mosquitto, PowerShell, then MQTTX', '<strong>Desk order:</strong> browser → Mosquitto installer → PowerShell to run the broker → MQTTX. Connect is allowed only now. Diagram by Koding Indonesia (FS-33).');
        $downloads = $this->figure('fs33-mosquitto-downloads.png', 'Official Eclipse Mosquitto download page at mosquitto.org/download', '<strong>This is the page to open.</strong> Scroll to <strong>Windows</strong>, then download the <code>x64.exe</code> file for a 64-bit PC. Do not download the Source (.tar.gz) archive for this lab. Source: <a href="https://mosquitto.org/download/" target="_blank" rel="noopener noreferrer">mosquitto.org/download</a> — Eclipse Mosquitto / Eclipse Foundation. Screenshot taken 13 August 2026. Mosquitto is licensed under the Eclipse Public License 2.0 and the Eclipse Distribution License 1.0.');
        ...
        $mqttx = $this->figure('fs33-mqttx-local.png', 'Illustration of MQTTX connected to Host 127.0.0.1 and Port 1883', '<strong>Connect is allowed now.</strong> Host = <code>127.0.0.1</code>, Port = <code>1883</code>. Do not copy a broker address from an internet screenshot. Illustration by Koding Indonesia (FS-33), modelled on the official <a href="https://mqttx.app/" target="_blank" rel="noopener noreferrer">MQTTX by EMQ</a> layout (Apache License 2.0). The official window screenshot is not used as-is because it shows a public broker.');
