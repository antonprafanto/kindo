...
HTML
            .$tools.<<<'HTML'
<ol><li><strong>Buka browser.</strong> Siapkan untuk mengunduh Mosquitto dari situs resminya.</li><li><strong>Buka MQTTX setelah Mosquitto berjalan.</strong> MQTTX sudah dipasang pada FS-32 dan akan menjadi alat uji kita. Jika belum ada, pasang dari <a href="https://mqttx.app/docs" target="_blank" rel="noopener noreferrer">dokumentasi resmi MQTTX</a>, lalu kembali ke langkah ini.</li><li><strong>Buka PowerShell hanya saat langkah Windows meminta perintah.</strong> Tidak perlu Arduino IDE, ESP32, kabel, IP router, atau akun broker publik.</li></ol>
...
HTML
            .$tools.<<<'HTML'
<ol><li><strong>Open a browser.</strong> Download Mosquitto from its official site.</li><li><strong>Open MQTTX after Mosquitto runs.</strong> MQTTX is the test tool installed in FS-32. If it is not installed yet, use the <a href="https://mqttx.app/docs" target="_blank" rel="noopener noreferrer">official MQTTX documentation</a>, then return here.</li><li><strong>Open PowerShell only when a Windows step requests a command.</strong> No Arduino IDE, ESP32, router IP, firewall change, or public-broker account is needed.</li></ol><p><strong>Security note:</strong> do not add a <code>listener</code>, change firewall rules, or open a router port. Home-network security and authentication come in FS-49.</p>
