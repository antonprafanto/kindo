...
class Article106Seeder extends Seeder
{
    ...
    private function body(): string
    {
        ...
        $format = $this->figure('fs36-format-fat32.png', 'Ilustrasi File Explorer: kartu dipilih, klik kanan Format, sistem berkas FAT32, tombol Mulai', '<strong>Klik kanan ikon kartu di File Explorer, lalu Format.</strong> Pilih sistem berkas <strong>FAT32</strong>, label boleh <code>FSIOT</code>, centang format cepat, lalu Mulai. Ilustrasi langkah Format, buatan Koding Indonesia (FS-36). Windows adalah merek Microsoft.');
        $wiring = $this->figure('fs36-wiring-spi.png', 'Wiring lengkap microSD SPI dan DHT22 ke ESP32: setiap pin ESP32 punya pasangan, termasuk GND modul SD', '<strong>Gambar utama — wiring.</strong> Tiap kabel punya pasangan: CS → GPIO 5, SCK → GPIO 18, MISO → GPIO 19, MOSI → GPIO 23, DHT22 DATA → GPIO 4, <strong>GND modul SD juga ke GND ESP32</strong>. VCC modul SD mengikuti label 5V atau 3V3. Urutan kaki fisik bisa berbeda antarmodul. Diagram buatan Koding Indonesia (FS-36).');
...
<li><strong>Mount</strong> — ESP32 berhasil membaca kartu. Kalau gagal, Serial menulis kalimat <code>Kartu tidak terbaca</code>.</li>
...
HTML
            .$wiring.$kitModule.$spiPhoto.$dhtPhoto.<<<'HTML'
...
<li>Diagram urutan tools, wiring, alur CSV, millis lawan NTP, skema periksa, serta ilustrasi Format, modul kit enam pin, dan Serial Monitor — Koding Indonesia (FS-36)</li>
</ul>
...
HTML;
    ...
    private function bodyEn(): string
    {
        ...
        $format = $this->figure('fs36-format-fat32.png', 'File Explorer illustration: card selected, right-click Format, FAT32 file system, Start button', '<strong>Right-click the card icon in File Explorer, then Format.</strong> Choose file system <strong>FAT32</strong>, the label may be <code>FSIOT</code>, quick format is fine, then Start. Format-step illustration by Koding Indonesia (FS-36). Windows is a trademark of Microsoft.');
        $wiring = $this->figure('fs36-wiring-spi.png', 'Complete microSD SPI and DHT22 wiring to ESP32: each ESP32 pin has a partner, including SD module GND', '<strong>Main figure — wiring.</strong> Each cable has a partner: CS → GPIO 5, SCK → GPIO 18, MISO → GPIO 19, MOSI → GPIO 23, DHT22 DATA → GPIO 4, and <strong>the SD module GND also goes to ESP32 GND</strong>. SD module VCC follows the 5V or 3V3 label. Physical pin order can differ between modules. Diagram by Koding Indonesia (FS-36).');
...
<li>Tool-order, wiring, CSV-flow, millis-versus-NTP, troubleshooting, Format, six-pin kit module, and Serial Monitor diagrams — Koding Indonesia (FS-36)</li>
</ul>
